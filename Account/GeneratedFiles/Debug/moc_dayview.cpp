/****************************************************************************
** Meta object code from reading C++ file 'dayview.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.5.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "stdafx.h"
#include "../../dayview.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'dayview.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.5.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_DayView_t {
    QByteArrayData data[7];
    char stringdata0[84];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_DayView_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_DayView_t qt_meta_stringdata_DayView = {
    {
QT_MOC_LITERAL(0, 0, 7), // "DayView"
QT_MOC_LITERAL(1, 8, 13), // "showTableView"
QT_MOC_LITERAL(2, 22, 0), // ""
QT_MOC_LITERAL(3, 23, 15), // "updateTableView"
QT_MOC_LITERAL(4, 39, 4), // "date"
QT_MOC_LITERAL(5, 44, 20), // "on_deleteBtn_clicked"
QT_MOC_LITERAL(6, 65, 18) // "on_viewBtn_clicked"

    },
    "DayView\0showTableView\0\0updateTableView\0"
    "date\0on_deleteBtn_clicked\0on_viewBtn_clicked"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_DayView[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    0,   34,    2, 0x0a /* Public */,
       3,    1,   35,    2, 0x0a /* Public */,
       5,    0,   38,    2, 0x08 /* Private */,
       6,    0,   39,    2, 0x08 /* Private */,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void, QMetaType::QDate,    4,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void DayView::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        DayView *_t = static_cast<DayView *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->showTableView(); break;
        case 1: _t->updateTableView((*reinterpret_cast< const QDate(*)>(_a[1]))); break;
        case 2: _t->on_deleteBtn_clicked(); break;
        case 3: _t->on_viewBtn_clicked(); break;
        default: ;
        }
    }
}

const QMetaObject DayView::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_DayView.data,
      qt_meta_data_DayView,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *DayView::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *DayView::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_DayView.stringdata0))
        return static_cast<void*>(const_cast< DayView*>(this));
    return QWidget::qt_metacast(_clname);
}

int DayView::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
